lib.callback.register("Ayse_VehicleSystem:getParkedVehicle", function(coords)
...
function setVehicleStolen(veh, status)
    DecorSetBool(veh, "AYSE_STOLEN_VEH", status)
end

function getVehicleStolen(veh)
    if not DecorExistOn(veh, "AYSE_STOLEN_VEH") then
        return false
    end
	return DecorGetBool(veh, "AYSE_STOLEN_VEH")
end

function setVehicleOwned(veh, status)
    DecorSetBool(veh, "AYSE_OWNED_VEH", status)
end

function getVehicleOwned(veh)
    if not DecorExistOn(veh, "AYSE_OWNED_VEH") then
        return false
    end
	return DecorGetBool(veh, "AYSE_OWNED_VEH")
end
...
function setVehicleLocked(veh, status)
    DecorSetBool(veh, "AYSE_LOCKED_VEH", status)
    SetVehicleDoorsLocked(veh, getDoorLock(status))
end

function getVehicleLocked(veh)
    if not DecorExistOn(veh, "AYSE_LOCKED_VEH") then
        return false
    end
	return DecorGetBool(veh, "AYSE_LOCKED_VEH")
end

function setVehicleEngine(veh, status)
    DecorSetBool(veh, "AYSE_ENGINE_VEH", status)
    SetVehicleEngineOn(veh, status, true, true)
end

function getVehicleEngine(veh)
    if not DecorExistOn(veh, "AYSE_ENGINE_VEH") then
        return false
    end
	return DecorGetBool(veh, "AYSE_ENGINE_VEH")
end
...
function lockpickVehicle()
    ...
    for i = 1, 7 do
        ...
        if not success or not DoesEntityExist(veh) or #(pedCoords - GetEntityCoords(veh)) > 2.5 then
            TriggerServerEvent("Ayse_VehicleSystem:syncAlarm", NetworkGetNetworkIdFromEntity(veh), false)
            finished = true
            return false, true
        end
        Wait(800)
    ...
    TriggerServerEvent("Ayse_VehicleSystem:syncAlarm", NetworkGetNetworkIdFromEntity(veh), true, "lockpick")
    return true, true
end
    ...
    local options = {
        {
            ...
            onSelect = function(args)
                ...
                TriggerServerEvent("Ayse_VehicleSystem:storeVehicle", vehid, properties)
            end
        }
    }
    for _, vehicle in pairs(vehicles) do
        if vehicle.available and (((garageTypes["water"] ~= vehicle.properties.class and garageTypes["heli"] ~= vehicle.properties.class and garageTypes["plane"] ~= vehicle.properties.class) and not garageTypes[garageType]) or menuClass == vehicle.properties.class) then
            ...
            options[#options + 1] = {
                ...
                onSelect = function(args)
                    local ped = PlayerPedId()
                    TriggerServerEvent("Ayse_VehicleSystem:takeVehicle", vehicle, garageLocation.vehicleSpawns)
                end,
                metadata = {
