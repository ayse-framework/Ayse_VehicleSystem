...
function transferVehicle(vehicleID, fromSource, toSource)
    local playerTo = AyseCore.Functions.GetPlayer(toSource)
...
function giveKeys(vehicle, source, target)
    ...
    local targetPlayer = AyseCore.Functions.GetPlayer(target)
...
function isParkingAvailable(source, coords)
    ...
    while tries < #coords do
        ...
        local available = lib.callback.await("Ayse_VehicleSystem:getParkedVehicle", source, vector3(coord.x, coord.y, coord.z))
        if available then
            return coord
        end
        tries += 1
...
function spawnOwnedVehicle(source, vehicleID, coords)
    ...
    for _, vehicle in pairs(vehicles) do
        if vehicle.owner == player.id and vehicle.id == vehicleID then
            ...
            TriggerClientEvent("Ayse_VehicleSystem:returnVehicles", source, getVehicles(player.id))
